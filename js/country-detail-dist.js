...
const callApiCountryDetail = async (id) => {
    try {
        const response = await fetch(
            `${_config__WEBPACK_IMPORTED_MODULE_0__.API_CONFIG.baseUrl}/countries/${id}?populate[0]=image&populate[1]=flag`,
            {
                method: 'GET',
                headers: {
        ...
        // set country data
        (0,_config__WEBPACK_IMPORTED_MODULE_0__.assignValueToNode)("coutry-name-breadcrumb", result.data?.name, "txt");
        (0,_config__WEBPACK_IMPORTED_MODULE_0__.assignValueToNode)("country-name-h1", result.data?.name, "txt-h1"); 
        (0,_config__WEBPACK_IMPORTED_MODULE_0__.assignValueToNode)("country-flag", result.data?.flag?.url, "image");
        (0,_config__WEBPACK_IMPORTED_MODULE_0__.assignValueToNode)("country-desc", result.data?.description, "html");
        (0,_config__WEBPACK_IMPORTED_MODULE_0__.assignValueToNode)("country-img", result.data?.image?.url, "image");
...
const callApiCountryCategory = async (id, name) => {
    try {
        const response = await fetch(
            `${_config__WEBPACK_IMPORTED_MODULE_0__.API_CONFIG.baseUrl}/country-categories?populate[0]=image&filters[country][id][$eq]=${id}`,
            {
                method: 'GET',
                headers: {
                    'Authorization': `Bearer ${_config__WEBPACK_IMPORTED_MODULE_0__.API_CONFIG.token}`
        ...
        result.data.forEach((category) => {
            ...
            countryCategory.innerHTML = `
                <a class="row border px-2 py-3 rounded card-link-box" id="country-category-link" href="../category?country-name=${name}&category-name=${(0,_config__WEBPACK_IMPORTED_MODULE_0__.formatedString)(category.name)}&id=${category.documentId}">\
...
const callApiCountry = async (sort, page) => {
    ...
    const uri = `${_config__WEBPACK_IMPORTED_MODULE_0__.API_CONFIG.baseUrl}/countries?populate[0]=flag&populate[1]=image&sort[0][${defaultParams.sortBy}]=${defaultParams.sortMethod}&pagination[page]=${defaultParams.pageCurr}&pagination[pageSize]=${defaultParams.pageSize}`;
    try {
...
const updateCountryList = (data) => {
    ...
    data.forEach((country) => {
        ...
        countryItem.innerHTML = `
            <a class="site-nav-item" href="/negara/detail?name=${(0,_config__WEBPACK_IMPORTED_MODULE_0__.formatedString)(country.name)}&id=${country.documentId}">${country.name}</a>
        `;
